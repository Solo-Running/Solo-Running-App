...
//            let startAnnotationImage = UIImage(systemName: "figure.walk.circle")
...
//            let endAnnotationImage = UIImage(systemName: "pin.fill")
...
//        }
    ...
    func convertMetersToString(distance: CLLocationDistance) -> String {
        ...
        if distance >= 304.8 {
            ...
            let miles = distance / metersInMile
            return String(format: "%.1f mi", miles)
        } else {
            // Convert meters to feet
            let feet = distance / metersInFoot
                ...
                MapReader { proxy in
                    ...
                    Map(position: $cameraPosition, interactionModes:  interactionModes, selection: $selectedPlaceMark) {
                        ...
                        if let places = locationManager.fetchedPlaces, !places.isEmpty {
                            ...
                            //                                if let stepCoordinates = manager.stepCoordinates, !manager.stepCoordinates.isEmpty {
                            //                                    ForEach(stepCoordinates, id: \.self) { item in
                        ...
                        VStack(alignment: .leading) {
                            ...
                            .confirmationDialog("Are you sure?", isPresented: $isShowingDeleteDialog) {
                                Button("Yes", role: .destructive) {
                                    ...
                                    //                                            switch result {
                                    //                                            case .success():
                                    //                                                runStatus = .planningRoute // TODO: remove later possibly
                                ...
                                .navigationDestination(isPresented: $canShowSummary) {
                                    RunSummaryView().environmentObject(locationManager).environmentObject(activityManager)
                                }
                            }
                            .frame(maxWidth: .infinity)
                            .padding(.horizontal, 16)
